...
- **Review mode gate:** Both exec summary and gap analysis delta displays are gated on `review_number > 0` (not just `previous_scores.length > 0`). First export saves scores with `review_number: 0` — delta only shows from the second export onward when there's a real prior to compare against.
- **Journey touchpoint details table removed:** The full touchpoint table was removed from `renderJourneyMap` — sentiment section already surfaces insights via arc + callouts. Journey map now renders only the stage summary table.
- **Roadmap improvement priority sort:** Improvements are sorted by priority (critical→high→medium→low) before building the step→improvement map so the highest-priority improvement wins when multiple link to the same step.
- **Value/waste split activation:** The cost summary value/waste section only renders when at least one step has `value_type` set via the step detail panel. Without classification, only the standard cost breakdown appears.
