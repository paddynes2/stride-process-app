...
- **Custom Radix DialogTitle wrapper breaks a11y check:** `ui/dialog.tsx` DialogTitle wraps a plain `<h2>`, not `DialogPrimitive.Title`. Radix DialogContent checks for `DialogPrimitive.Title` via React context — a plain `<h2>` is invisible to this check, causing console.error. Fix: use `DialogPrimitive.Title` directly, or change ui/dialog.tsx to wrap `DialogPrimitive.Title`. (BUG-023, iter 102)
...
- **Builder `git add -f` bypasses .gitignore (iter 98):** FEAT-052 builder used `git add -f` to force-stage files under `autonomous-dev/.ralph/worktrees/build-1/src/...` (which is in .gitignore). This committed 7 files to worktree-relative paths instead of `src/`. Files exist in git but are unreachable at runtime. Pipeline must never use `git add -f` — if files are in .gitignore, the pipeline should copy them to the correct location first. Slot 2 (#BUG-020) had a different failure: builder claimed completion but no commit exists in git log at all.

- **API 500 errors likely from unpushed migrations (iter 112):** /api/v1/improvement-ideas and /api/v1/coloring-rules both return 500 for authenticated users. Most likely cause: migrations 019 (coloring_rules table) and 022 (improvement_ideas table) not applied to local dev DB. `npx supabase db push` may fix both. Same root cause as BUG-022 (templates table missing from remote). TypeScript compiles fine because types exist in code, but runtime Supabase queries fail against missing tables.
