...
- **API 500 errors from unpushed migrations (iter 112) — RESOLVED (iter 117+):** Migrations 018-023 pushed to remote Supabase DB. Root cause confirmed: tables didn't exist in remote schema. Additional fix required: `uuid_generate_v4()` → `gen_random_uuid()` in migrations 019, 020, 022 because Supabase installs uuid-ossp in the `extensions` schema (not `public`), making `uuid_generate_v4()` unavailable. `gen_random_uuid()` is built into PG 13+ with no extension needed.
