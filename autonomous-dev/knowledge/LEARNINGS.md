...
- ~~**MATURITY_COLORS duplication:** The `MATURITY_COLORS` map (1→red through 5→green) is defined in step-node.tsx, section-node.tsx, and flow-canvas.tsx (legend). If a 4th usage appears, extract to a shared `lib/constants.ts` or `lib/maturity.ts` file.~~ [SUPERSEDED by: MATURITY_COLORS extracted, iteration 29]
...
- ~~**MATURITY_COLORS duplication now in 4 places [SUPERSEDES: MATURITY_COLORS duplication]:** The map is now in step-node.tsx, section-node.tsx, flow-canvas.tsx (legend), AND `src/lib/export/pdf.ts`. Should extract to `lib/constants.ts` — add to IMPROVEMENTS.md.~~ [SUPERSEDED by: MATURITY_COLORS extracted to `src/lib/maturity.ts`, iteration 29]
...
- **Dynamic import for export-only deps:** jspdf + html-to-image should always be dynamically imported since export is a rare user action. In canvas-view.tsx, `const { exportWorkspacePdf } = await import("@/lib/export/pdf")` inside the callback keeps these out of the initial bundle. Same pattern for png.ts.
- **next/dynamic for named exports:** When using `next/dynamic` with a named export (like `RichTextEditor`), wrap the import: `dynamic(() => import("./file").then(m => ({ default: m.NamedExport })), { ssr: false })`. The `ssr: false` option is also useful for components that depend on browser APIs.
- **matchMedia for responsive collapse:** Use `window.matchMedia("(max-width: Npx)")` with `addEventListener("change", handler)` for responsive state changes in React. More reliable than resize listeners and matches CSS media query behavior exactly. Clean up with `removeEventListener` in useEffect return.
- **Tailwind responsive in this codebase:** Only workspace-list.tsx used responsive breakpoints (sm:, lg:). All other grid layouts (gap-analysis, teams summary cards) were hardcoded grid-cols-3. When adding responsive support, check loading.tsx skeletons too — they should match the responsive grid of the actual view.
- **React Compiler stricter in app/ directory:** The React Compiler lint rule (react-hooks/purity) flags `Math.random()` as impure in components located under `app/` (route-level files) but not in `components/`. For journey-canvas-view.tsx, use deterministic positioning (grid offset based on count) instead of `Math.random()`. Also, self-referential retry callbacks in `useCallback` are flagged as "cannot access variable before declaration."
